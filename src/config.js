...
// 连带把 voice/security 等兄弟字段一起重置"（升级后最常见的"配置全没了"根因）。
...
// activate() 等写操作必须基于它合并，否则会抹掉 voice/security 等其它字段。
...
function runConfigMigrations() {
  ...
  if (from >= CONFIG_SCHEMA_VERSION) {
    // 兼容曾被旧版设置接口重新写回 schema v3 config.json 的 voice 块。
    // 即使版本号已是最新，也要自愈迁移，否则后端运行时只读 voice/<provider>.json 时拿不到密钥。
    if (Object.keys(readLegacyVoiceBlock(parsed)).length) {
      try {
        const healed = migrateLegacyVoiceConfig(parsed)
        writeStoredConfig({ ...healed, schemaVersion: CONFIG_SCHEMA_VERSION })
        console.log('[config] 已自愈迁移遗留 voice 配置到独立厂商文件')
      } catch (e) {
        console.warn('[config] 自愈迁移遗留 voice 配置失败，已保留原文件:', e.message)
      }
    }
    return
  }
...
function validateVoiceConfigUpdates(updates, existing) {
  const aliyunApiKey = String(updates?.aliyunApiKey || '').trim()
  if (!aliyunApiKey) return
  if (!isValidAliyunAsrKey(aliyunApiKey)) {
    throw new Error('\u963f\u91cc\u4e91 ASR API Key \u683c\u5f0f\u4e0d\u6b63\u786e\uff1a\u8bf7\u586b\u5199\u767e\u70bc/DashScope \u63a7\u5236\u53f0\u4e2d sk- \u5f00\u5934\u7684 API Key')
  }
  if (
    existing.apiKey &&
    aliyunApiKey === existing.apiKey &&
    CHAT_PROVIDERS_WITH_AMBIGUOUS_SK_KEYS.has(existing.provider)
  ) {
    throw new Error('\u8be5\u5bc6\u94a5\u4e0e\u5f53\u524d\u804a\u5929\u6a21\u578b\u5bc6\u94a5\u76f8\u540c\uff0c\u65e0\u6cd5\u786e\u8ba4\u5b83\u662f\u963f\u91cc\u4e91\u767e\u70bc ASR Key\uff1b\u8bf7\u586b\u5199\u767e\u70bc/DashScope \u7684\u72ec\u7acb API Key')
  }
}

export function setVoiceConfig(updates) {
  const existing = readExistingStoredConfig()
  validateVoiceConfigUpdates(updates, existing)
