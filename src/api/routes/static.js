...
const RUNTIME_ASSET_ROOT = path.join(paths.resourcesDir, 'src', 'runtime')
const MAP_ASSET_ROOT = path.join(paths.resourcesDir, 'public', 'maps')
const ECHARTS_BUNDLE_PATH = path.join(paths.resourcesDir, 'node_modules', 'echarts', 'dist', 'echarts.min.js')
...
export async function handleStaticRoutes(req, res, url) {
  ...
  if (req.method === 'GET' && (url.pathname === '/' || url.pathname === '/index.html')) {
    // 启动后默认打开完整管理后台（包含地图、侧边栏、机甲面板等），
    // 而不是只引入 app.js 的精简 index.html。
  ...
  if (req.method === 'GET' && url.pathname.startsWith('/maps/')) {
    serveAsset(req, res, MAP_ASSET_ROOT, '/maps/')
    return true
  }

  if (req.method === 'GET' && url.pathname.startsWith('/src/runtime/')) {
    serveAsset(req, res, RUNTIME_ASSET_ROOT, '/src/runtime/')
    return true
  }
