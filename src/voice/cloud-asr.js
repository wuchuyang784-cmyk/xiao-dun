...
import { getVoiceRuntimeConfig, normalizeVoiceProvider } from '../config.js'
...
export function resolveCloudASRSessionConfig(request = {}) {
  const runtime = getVoiceRuntimeConfig(request?.provider || null)
  return {
    ...runtime,
    provider: runtime.provider || runtime.voiceProvider || normalizeVoiceProvider(request?.provider || 'aliyun'),
    lang: request?.lang || runtime.lang || 'zh',
  }
}

export function routeCloudASRClientPayload(session, raw, isBinary) {
  if (isBinary) {
