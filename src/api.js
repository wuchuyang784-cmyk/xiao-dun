...
import { dismissSceneSurfaceForIntent } from './scene/scene-intent-handler.js'
...
import { createCloudASRSession, resolveCloudASRSessionConfig, routeCloudASRClientPayload } from './voice/cloud-asr.js'
...
import { handleContextRoutes } from './api/routes/context.js'
...
function getApiHost() {
  ...
  const envHost = String(globalThis.process?.env?.XIAODUN_HOST || '').trim()
...
function isLanAccessEnabled() {
  ...
  return getNetworkConfig().allowLanAccess
    ...
    || /^(1|true|yes|on)$/i.test(String(globalThis.process?.env?.XIAODUN_ALLOW_LAN || '').trim())
...
function getAuthToken() {
  ...
  return String(globalThis.process?.env?.XIAODUN_API_TOKEN || '').trim()
...
async function dispatchHttpRoutes(req, res, url, context) {
  ...
  if (await handleContextRoutes(req, res, url)) return true
    ...
    ws.on('message', (raw, isBinary) => {
      ...
      if (!configured) {
        ...
        try {
          ...
          const runtimeConfig = resolveCloudASRSessionConfig(msg)
      ...
      routeCloudASRClientPayload(session, raw, isBinary)
...
function attachSceneProtocol() {
  ...
  setSceneIntentHandler((msg) => {
    ...
    if (dismissSceneSurfaceForIntent(msg)) return
...
export function startAPI(port = 3721, { getStateSnapshot = null, onActivated = null } = {}) {
  ...
  const readyFile = String(globalThis.process?.env?.XIAODUN_WEB_READY_FILE || '').trim()

  const removeReadyFile = () => {
  ...
  server.listen(port, host, () => {
    ...
    if (readyFile) {
      ...
      try {
        ...
        fs.writeFileSync(readyFile, JSON.stringify({ host, port, pid: process.pid, readyAt: new Date().toISOString() }), 'utf8')

      } catch (error) {

        console.error('[API] Failed to write web ready marker:', error.message)
  ...
  if (readyFile) {
    ...
    server.once('close', removeReadyFile)
