...
function getDesktopWindowLayoutSnapshot() {
  try {
    const reader = globalThis?.getXiaoDunWindowLayoutSnapshot
...
function execCapabilityDemo(args = {}, context = {}) {
  ...
  const introText = runCapabilityDemo({
  ...
  return toolJson({
    ...
    final_reply_guidance: 'The intro message has already been sent while the visual demo starts. End the round now; do not send another introduction.',
  })
}
...
function execOpenDocPanel(args = {}) {
  ...
  const validTopics = ['voice_asr', 'voice_config']
...
function execTerminalStream({
  ...
  const cleanTitle = String(title || 'XiaoDun Terminal Stream').trim() || 'XiaoDun Terminal Stream'
