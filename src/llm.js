...
// 客户端并发闸门：限制同一账号同时「在途」的 LLM 请求数。
// 服务商（MiniMax / 智谱 / MiMo 等）普遍按账号设并发上限，超出即被拒或排队，
// 表现为 "Concurrency limit exceeded" 或长时间无数据（被 45s 空闲超时掐断并重试）。
// 这里在客户端把并发收敛到 N，超出部分本地排队（FIFO），从源头消除该错误与重试风暴。
// 若仍偶发并发超限，把 XIAODUN_LLM_MAX_CONCURRENCY 调成 1（严格串行）即可。
const LLM_MAX_CONCURRENCY = (() => {
  const raw = Number(process.env.XIAODUN_LLM_MAX_CONCURRENCY)
  if (!Number.isFinite(raw)) return 2
  return Math.min(8, Math.max(1, Math.floor(raw)))
})()
let _llmInflight = 0
const _llmWaitQueue = []
function _llmAcquire(signal) {
  return new Promise((resolve, reject) => {
    const onAbort = () => {
      const idx = _llmWaitQueue.indexOf(take)
      if (idx >= 0) _llmWaitQueue.splice(idx, 1)
      reject(Object.assign(new Error('Aborted'), { name: 'AbortError' }))
    }
    const take = () => {
      if (_llmInflight < LLM_MAX_CONCURRENCY) {
...
async function streamOnce({ messages, toolSchemas, temperature, topP, maxTokens, thinking = true, signal, onStream, model = config.model }) {
  ...
  // 客户端并发闸门：同一账号在途请求不超过 LLM_MAX_CONCURRENCY，避免触发服务商并发上限被拒/排队。
  await _llmAcquire(signal)
  armIdle() // 拿到槽位后才开始空闲计时，排队等待期间不触发 45s 超时
...
function isTransientError(err) {
  ...
  return /timeout|timed out|socket hang up|fetch failed|network error|upstream|concurrency|rate[\s_-]?limit|too many requests|busy|overload|resource exhausted|quota|please retry later/i.test(msg)
}
...
// 出现"工具调不动 + 嘴也被堵住"的死锁（lessons-xiaodun-silent-exit 的镜像问题）。
...
// ——用户那边就是"同一个问题被回答两遍、第二遍像重启"。该现象的原回归用例已随测试清理移除。
//
      ...
      if (stopReason) {
        ...
        if (actionContractBlocksSend) {
          ...
          result = JSON.stringify({
            ...
            reason: 'You already acknowledged the playback with a single emoji this turn (and the system told the user when you started looking for it). This further play-confirmation is redundant — the player is visibly running. The runtime suppressed it. For video playback: one emoji at most after a successful play, then just end the round.',
          })
