...
export const SELF_KNOWLEDGE_TOPICS = {
  ...
  self_architecture: {
    ...
    subtitle: 'How XiaoDun Works',

    icon: '⚙',

    summary: `小盾（XiaoDun）是一套 Electron + Node.js 的"持续意识"框架，当前版本 ${APP_VERSION}。它不是被动等待提问的聊天机器人，而是一个持续运行、自主感知、带长期记忆的 Agent。以下是当前版本的完整机制说明。`,
...
数据落在 SQLite（src/db.js）与 data/ 目录；运行配置在 config.json + 若干独立配置文件。`,
...

      {
        ...
        content: `小盾不是"两个人格"，而是同一个 AI 的两种触发入口，共享同等的上下文质量（记忆、人物卡、思维、UI 状态）：
...
配套的"成果审视分身"会在收尾前复查（见下）。`,
...

      {
        ...
        content: `小盾的记忆不是简单的"存一段查一段"，而是一套"一切皆记忆 / 少即是强"的动态池——目标是每轮注入"合适的上下文"，不是"召回越多越好"。
...
Key 配置：serper / brave / tavily / jina / searxng，存在 config.json 顶级字段或环境变量。`,
...

      {
        ...
        content: `\u8bed\u97f3\u6a21\u5757\u4ec5\u8d1f\u8d23 ASR\uff08\u8bed\u97f3\u8bc6\u522b\uff09\uff1a

  - \u9ea6\u514b\u98ce\u91c7\u96c6\u4e0e\u4e91\u7aef\u5b9e\u65f6\u8bc6\u522b

  - \u652f\u6301\u963f\u91cc\u4e91\u3001\u817e\u8baf\u4e91\u3001\u79d1\u5927\u8baf\u98de\u3001\u706b\u5c71\u5f15\u64ce\u548c\u672c\u5730 Whisper

  - \u957f\u8bed\u97f3\u5305\u542b\u8f6c\u5f55\u53bb\u91cd\u3001\u97f3\u9891\u91cd\u8fde\u8865\u53d1\u548c\u8bc6\u522b\u72b6\u6001\u7ba1\u7406

  - Agent \u56de\u590d\u4ee5\u6587\u5b57\u663e\u793a\uff0c\u4e0d\u63d0\u4f9b\u8bed\u97f3\u5408\u6210\u6216\u8bed\u97f3\u64ad\u62a5\u3002`,
...
配置见"微信 / 社交平台配置"文档主题（wechat_config）。`,
  ...
  ui_design: {
    ...
    subtitle: 'XiaoDun UI & Scene Design',

    icon: '🖥',

    summary: '小盾的界面叫 Brain UI，运行在 Electron 渲染进程。Agent 通过声明式 Scene 协议驱动界面（UI = f(scene)），并能感知界面状态。以下是界面各部分的设计说明。',
...
原则：仅当"可视化比纯文字更清楚"时才 ui_set，普通问答不主动投影。`,
  ...
  - 托盘后台运行，不抢主窗口焦点`,
...

      {
        ...
        content: `小盾界面有一套统一的视觉规范，新增 UI 必须遵循：
...
`,
...

      {
        ...
        content: `Turn Trace 回合追踪：逐回合记录每一轮真实喂给 LLM 的 messages[] 与思考过程，user/tool 消息会标出 agent 名并标红，专查"角色归属混乱""镜像复读"等生成层问题。数据由 runtime/turn-trace.js 用 offset 还原以省内存，经 /admin/traces 提供。`,
...
export function detectSelfKnowledgeTopic(text) {
  ...
  if (
    ...
    /(你的代码|你.*怎么运行|你.*怎么工作|你.*架构|你.*如何运作|小盾.*代码|xiaodun.*代码|你.*实现|代码机制|运行机制|技术架构|你.*内部|你.*系统|你.*模块|你.*是怎么|你.*如何思考|你.*心跳|意识循环|认知循环|动态记忆|记忆池|审视分身|ticker|queue\.js|control\.js|llm\.js|prompt\.js|memory.*机制|记忆.*(系统|机制)|工具.*调用|capability|executor|l1.*l2|l2.*l1|两个入口|react.*任务|self.?knowledge|自知识|自我感知)/.test(
