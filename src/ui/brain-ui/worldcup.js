...
// 重播入场动画：复用热点同款 hs-glitch-in（见 styles.css 的 .worldcup-boot 规则）
function replayWorldcupBoot() {
  const panel = document.querySelector('.worldcup-panel')
  if (!panel) return
  panel.classList.remove('worldcup-boot')
  void panel.offsetWidth
  panel.classList.add('worldcup-boot')
}
...
export function setWorldcupMode(visible, { source = 'brain-ui' } = {}) {
  ...
  if (nextVisible) {
    ...
    for (const mode of ['video-mode', 'image-mode']) {
    ...
    document.querySelector('.worldcup-panel')?.classList.remove('worldcup-boot');
  ...
  window.dispatchEvent(new CustomEvent('xiaodun:worldcup-mode', {
...
export async function initWorldcup() {
  ...
  window.addEventListener('xiaodun:hotspot-mode', (event) => {
  ...
  window.addEventListener('message', (event) => {
    ...
    if (phase === 'down') {
      window.xiaodunVoice?.pttStart?.();
      expandConsole();                        // 说话时展开看实时识别文字
    } else if (phase === 'up') {
      window.xiaodunVoice?.pttEnd?.();
      scheduleConsoleCollapse(MESSAGE_PEEK_MS);
    } else if (phase === 'cancel') {
      window.xiaodunVoice?.pttEnd?.({ send: false });
