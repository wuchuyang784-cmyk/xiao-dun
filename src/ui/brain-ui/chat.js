...
export function initChat({
  ...
  function findMessageElement(messageId) {
    ...
    return Array.from(chatMessages.querySelectorAll("[data-message-id]")).find(
  ...
  function claimRenderedMessage({ messageId, role, text, label, source = "event", dedupe = true } = {}) {
    ...
    // The DOM is the source of truth. History trimming can remove an old

    // message while its id remains in the in-memory set; do not let that stale

    // entry suppress a later real event.

    if (id && renderedMessageIds.has(id)) {

      if (findMessageElement(id)) return false;
  ...
  msgInput.addEventListener("keydown", event => {
    ...
    if (event.key === "k" && (event.ctrlKey || event.metaKey) && !event.shiftKey && !event.altKey) {

      event.preventDefault();

      triggerContextCompress();
  ...
  // 输入框以 "/" 开头时弹出命令菜单。ASR/LLM 直接打开对应设置面板；
  ...
  const SLASH_COMMANDS = [
...

    {
      ...
      run: () => triggerContextCompress(),
    },
    {
      cmd: "/context", keys: ["context", "ctx", "上下文", "用量"],
      label: "上下文用量", desc: "查看当前 LLM 上下文估算的 token 数与消息数",
      run: () => showContextStats(),
    },
  ...
  async function showContextStats() {
    try {
      const res = await fetch('/api/v1/context/stats')
      const env = await res.json()
      const d = env.data || {}
      addMsg('jarvis',
        `📊 **上下文用量**\n\n` +
        `· 消息数：${d.messageCount ?? '?'}\n` +
        `· 估算 token：${d.estimatedTokens ?? '?'}\n` +
        (d.lastTimestamp ? `· 最近一条：${d.lastTimestamp}\n` : '') +
        `\n上下文超过 16000 token 建议 \`/compress\` 压缩`,
        { alert: false, pending: false })
      openChat()
    } catch (err) {
      addMsg('jarvis', `❌ 上下文用量查询失败：${err.message}`, { alert: false, pending: false })
    }
  }

  async function triggerContextCompress() {
    try {
      addMsg('jarvis', '⏳ 正在压缩上下文...', { alert: false, pending: false })
      const res = await fetch('/api/v1/context/compress', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      addMsg('jarvis', `❌ 压缩失败：${err.message}`, { alert: false, pending: false })
    }
  }
  ...
  function finalizeLiveJarvisMsg(text, options = {}) {
    ...
    if (typeof text === "string") {
      ...
      const existing = findMessageElement(options.messageId);

      if (existing && existing !== liveEl) {
      ...
      if (!claimRenderedMessage({ messageId: options.messageId, role: "jarvis", text, label: labelText, source: options.source || "event" })) {
        ...
        // If the id was claimed by a stale DOM entry, keep the existing message;

        // otherwise allow the current live bubble to become the authoritative one.

        if (findMessageElement(options.messageId)) {
        ...
        const staleId = normalizeMessageId(options.messageId);

        if (staleId) renderedMessageIds.delete(staleId);

        if (!claimRenderedMessage({ messageId: options.messageId, role: "jarvis", text, label: labelText, source: options.source || "event", dedupe: false })) {
  ...
  return {
    ...
    hasRenderedMessage: (messageId) => !!findMessageElement(messageId),
