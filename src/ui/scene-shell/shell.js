...
export class Shell {
  ...
  _buildSurface(surface, { dismissible = false } = {}) {
    ...
    if (dismissible) {
      ...
      close.type = 'button'
      close.className = 'surface-dismiss'
      close.setAttribute('aria-label', '关闭此卡片')
      close.title = '关闭'
      close.textContent = '×'
      close.addEventListener('click', (event) => {
        event.stopPropagation()
        this.onIntent({ surface: surface.id, name: 'dismiss', data: {} })
      })
